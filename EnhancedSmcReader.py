# Enhanced ModifiedSMCReader.py with additional camera parameter utilities
import h5py
import cv2
import numpy as np 
...
class SMCReader:
    ...
    def get_camera_info_by_id(self, camera_id):
        """
        根据相机ID获取相机信息
        
        Args:
            camera_id (int/str): 相机ID
            
        Returns:
            dict: 相机信息字典，包含分辨率等信息
        """
        camera_id = int(camera_id)
        
        if camera_id < 48:  # 5MP相机
            return self.Camera_5mp_info
        else:  # 12MP相机
            return self.Camera_12mp_info
    ...
    def get_camera_ids(self):
        ...
        Returns:
            list: 相机ID列表
        """
        if 'Camera_Parameter' not in self.smc:
            return []
        
        return sorted([int(cam_id) for cam_id in self.smc['Camera_Parameter'].keys()])
    
    def get_camera_groups_info(self):
        """
        获取所有相机组的详细信息
        ...
        if self.Camera_5mp_info:
            info['Camera_5mp'] = self.Camera_5mp_info
            info['Camera_5mp']['id_range'] = '0-47'
            
        if self.Camera_12mp_info:
            info['Camera_12mp'] = self.Camera_12mp_info
            info['Camera_12mp']['id_range'] = '48-60'
            
        if self.Kinect_info:
            info['Kinect'] = self.Kinect_info
    ...
    def get_calibration_with_world_transform(self, Camera_id):
        """
        获取相机标定参数并计算世界坐标变换
        ...
        Returns:
            dict: 包含原始标定参数和世界坐标变换的字典
        """
        calibration = self.get_Calibration(Camera_id)
        if not calibration or calibration.get('RT') is None:
            return calibration
        
        # 计算世界坐标变换
        RT_matrix = calibration['RT']
        ...
        # 添加世界坐标相关信息
        calibration['RT_world'] = camera_to_world
        calibration['camera_position'] = camera_to_world[:3, 3]
        calibration['camera_orientation'] = camera_to_world[:3, :3]
        
        # 添加相机参数
        if calibration.get('K') is not None:
            K = calibration['K']
            camera_info = self.get_camera_info_by_id(Camera_id)
    ...
    def get_camera_statistics(self):
        ...
        if self.Camera_5mp_info:
            stats['camera_5mp_count'] = self.Camera_5mp_info['num_device']
            stats['total_cameras'] += stats['camera_5mp_count']
            
        if self.Camera_12mp_info:
            stats['camera_12mp_count'] = self.Camera_12mp_info['num_device']
            stats['total_cameras'] += stats['camera_12mp_count']
        
        # 检查标定数据
        if 'Camera_Parameter' in self.smc:
            all_